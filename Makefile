...
# This is for simgrid; these options will not work for others, probably
MPICXX = smpicxx
#MPICXX ?= mpicxx
...
quick : $(TARGETS)
	smpirun -hostfile topologies/hostfile-16.txt -platform topologies/torus-2-2-4.xml -np 16 --cfg=smpi/host-speed:20000000 --cfg=smpi/reduce:ompi ./dotprod_mpi $(VECLEN) torus-2-2-4
	smpirun -hostfile topologies/hostfile-16.txt -platform topologies/fattree-16.xml -np 16 --cfg=smpi/host-speed:20000000 --cfg=smpi/reduce:ompi ./dotprod_mpi $(VECLEN) fattree-16
	smpirun -hostfile topologies/hostfile-72.txt -platform topologies/fattree-72.xml -np 72 --cfg=smpi/host-speed:20000000 --cfg=smpi/reduce:ompi ./dotprod_mpi $(VECLEN) fattree-72
	smpirun -hostfile topologies/hostfile-72.txt -platform topologies/torus-2-4-9.xml -np 72 --cfg=smpi/host-speed:20000000 --cfg=smpi/reduce:ompi ./dotprod_mpi $(VECLEN) torus-2-4-9
...
sim : $(TARGETS)
	$(foreach algo,$(MPI_REDUCE_ALGOS), \
		smpirun -hostfile topologies/hostfile-$(NUM_PROCS).txt -platform topologies/fattree-$(NUM_PROCS).xml -np $(NUM_PROCS) \
			...
			$(LOG_LEVEL) \
			./dotprod_mpi $(VECLEN_BIG) fattree-$(NUM_PROCS);)
...
test : $(TARGETS)
	...
	$(foreach algo,$(ALGOS),\
		echo Reduction algorithm $(algo) ; \
		mpirun -np $(NUM_PROCS) --mca $(VERBOSITY) --mca coll_tuned_reduce_algorithm $(algo) dotprod_mpi $(VECLEN) $(algo);)
